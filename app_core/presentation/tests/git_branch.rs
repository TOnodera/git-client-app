...
use domain::{command::command::{GitBranchCommandTrait, GitBranchCommandInput, GitBranchCommandOption}, types::Result, service::GitBranchServiceTrait};
...
fn git_branchコマンドを実行してBranchオブジェクトの配列を取得できる() -> Result<()>{
    ...
    let result = usecase.run(None::<GitBranchCommandInput>, None::<GitBranchCommandOption>)?;
